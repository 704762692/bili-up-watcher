...
数据源策略(GitHub 云服务器直连 B站接口常被 412 风控,已内置对策):
  1. 主源:B站官方动态接口。请求前先通过 finger/spi 接口取匿名 cookie(buvid3/4),
     再带 cookie+浏览器头请求;遇到 412 自动换新 cookie 重试一次。
  2. 备用:RSSHub 公共实例(服务器端自己处理风控,本机只读它的结果)。

用法:
  1. UID 与 UP名 已填在下方"基础配置"
  2. 推送渠道任选其一,填 key 即启用(推荐微信 PushPlus)
  3. 本文件与 .github/workflows/monitor.yml 一起放在 GitHub 仓库,自动每 5 分钟运行
...
# 备用数据源:公共 RSSHub 实例列表。官方接口失败时自动轮着试。
# 可自行添加更多可用实例(参考 https://docs.rsshub.app/zh/guide/instances)
...
def _http_get(op, url, headers, timeout=20):
...
# ============================================================
# 匿名 cookie(解决 412 风控):优先 finger/spi 接口,失败则试首页
# ============================================================

def get_buvid(op, force=False):
    """获取匿名 cookie 字典 {buvid3/buvid4/b_nut}。force=True 时跳过首页直接取新 spi。"""
    cookies = {}
    if not force:
        try:
            ...
            with op.open(req, timeout=20) as r:
                for line in (r.headers.get_all("Set-Cookie") or []):
                    kv = line.split(";", 1)[0]
                    k, _, v = kv.partition("=")
                    if k.strip() in ("buvid3", "b_nut"):
                        cookies[k.strip()] = v.strip()
        except Exception:
            pass
    try:
        raw = _http_get(op, "https://api.bilibili.com/x/frontend/finger/spi",
...
# ============================================================
# WBI 签名(2023 年后 B站接口强制;匿名请求 nav 即可获取密钥)
# ============================================================
...
_WBI_CACHE = {"mixin": None}


def get_wbi_mixin(op):
    """匿名获取 WBI 混淆密钥(本轮运行内缓存)。失败返回 None。"""
    if _WBI_CACHE["mixin"]:
        return _WBI_CACHE["mixin"]
    mixin = None
    try:
        raw = _http_get(op, "https://api.bilibili.com/x/web-interface/nav",
                        {"User-Agent": UA, "Referer": "https://www.bilibili.com/"})
        data = json.loads(raw.decode("utf-8", "ignore"))
        wbi = (data.get("data") or {}).get("wbi_img") or {}
        img = wbi.get("img_url", "").split("/")[-1].split(".")[0][-32:]
        sub = wbi.get("sub_url", "").split("/")[-1].split(".")[0][-32:]
        if len(img) == 32 and len(sub) == 32:
            mixin = "".join((img + sub)[i] for i in MIXIN_TAB)[:32]
    except Exception as e:
        print("  获取 WBI 密钥失败:%s" % e)
    _WBI_CACHE["mixin"] = mixin
    return mixin
...
# ============================================================
# 抓取:主=官方新接口(带cookie+自动重试);备=RSSHub
# ============================================================

def parse_polymer(data):
    """解析官方新接口返回,统一格式列表(新→旧)。"""
    items = []
    for it in (data.get("data", {}).get("items") or []):
        ...
        link = ("https://www.bilibili.com/video/%s" % bvid
                ...
                else "https://www.bilibili.com/opus/%s" % did)
        items.append({
            "id": did, "ts": ts, "type_name": type_name(dtype),
            "text": trunc(text, 260), "link": link,
        })
...
def fetch_polymer(uid, op):
    """官方新动态接口(主源):WBI 签名 + buvid cookie,失败自动刷新重试一次。"""
    cookies = get_buvid(op)
    last_err = None
    for attempt in (1, 2):
        params = {"host_mid": str(uid), "platform": "web",
                  "timezone_offset": "-480", "features": "itemOpusStyle",
                  "wts": str(int(time.time()))}
        mixin = get_wbi_mixin(op)
        if mixin:
        ...
        try:
            raw = _http_get(op, url, headers, timeout=25)
            data = json.loads(raw.decode("utf-8", "ignore"))
            if data.get("code") != 0:
                raise RuntimeError("B站接口 code=%s: %s" % (data.get("code"), data.get("message")))
            items = parse_polymer(data)
            if not items:
                raise RuntimeError("接口返回空 items")
            return items
            ...
            last_err = str(e)
        if attempt == 1:
            print("  请求被拦(%s),刷新 cookie 与 WBI 密钥重试一次…" % last_err)
            cookies = get_buvid(op, force=True)
            _WBI_CACHE["mixin"] = None
            time.sleep(1)
    raise RuntimeError("官方接口失败: %s" % last_err)
...
def fetch_rss(uid, instances):
    ...
    for base in instances:
        try:
            ...
            raw = _http_get(urllib.request.build_opener(), url,
                            {"User-Agent": UA, "Accept": "application/xml, application/rss+xml, */*"},
                            timeout=25)
            ...
            for item in root.iter("item"):
                ...
                if not did:
                    continue
                kw = re.search(r"(视频|图文|动态|转发|文章|笔记)", title)
                items.append({
                    "id": did, "ts": int(time.time()),
            ...
            print("  RSS源 %s 不可用:%s" % (base, e))
    return []


def fetch_dynamics(uid, op):
    try:
        return fetch_polymer(uid, op)
    except Exception as e:
        print("  主源失败:%s → 尝试 RSSHub 备用源" % e)
    items = fetch_rss(uid, RSSHUB_INSTANCES)
    if items:
        return items
    raise RuntimeError("官方接口与 RSSHub 备用源均不可用(可能被临时风控,下次运行自动重试)")
